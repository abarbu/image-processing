...
(define (pnm-rotate pnm)
 (cond ((pbm? pnm)
	(make-pbm (pbm-raw? pnm) (transpose-matrix (pbm-bitmap pnm))))
       ((pgm? pnm)
	(make-pgm (pgm-raw? pnm)
		  (pgm-maxval pnm)
		  (transpose-matrix (pgm-grey pnm))))
       ((ppm? pnm)
	(make-ppm (ppm-raw? pnm)
		  (ppm-maxval pnm)
		  (transpose-matrix (ppm-red pnm))
		  (transpose-matrix (ppm-green pnm))
		  (transpose-matrix (ppm-blue pnm))))
       (else (error "Argument is not a PNM"))))
